...
export function process(extValue: mixed, schema: Schema): {
  ...
  while (doLoop) {
    ...
    if (!lastStackEntry) {
      ...
      if (lastStackEntry.type === 'array') {
        const extArray: Array<mixed> = lastStackEntry.extValueCurrent;
        ...
        if (schemaCurrent.type === 'object') {
          ...
          const valueToUse: { [string]: mixed, } = schemaCurrent.getStub();
          stack.push({
            type: 'object',
            schema: schemaCurrent,
            valueToUse,
